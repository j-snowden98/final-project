...
async function availableResidents() {
  ...
  const sql = await init();
  const query = sql.format('SELECT X.id, CONCAT(CONCAT(X.forename, " "), X.surname) AS resName FROM Resident X LEFT JOIN (SELECT roomID, resID FROM ResidentRoom) Y ON Y.resID = X.id WHERE Y.roomID IS NULL ORDER BY X.forename, X.surname ASC');
  const [rows] = await sql.query(query);
  return rows;
}
